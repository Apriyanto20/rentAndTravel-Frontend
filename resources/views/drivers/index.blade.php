    ...
    <div class="py-12">
        <div class="max-w-9xl mx-auto sm:px-6 lg:px-8">
            <div class="flex items-start justify-between gap-5">
                <div class="bg-white px-10 pt-5 pb-6 rounded-3xl border shadow-xl w-full">
                    <div class="flex items-center justify-between w-full">
                        <div class="flex items-center gap-3 w-full">
                            <div class="w-12"><img src="{{ url('/assets/img/data.png') }}" alt=""
                                    srcset="">
                            </div>
                            <div class="font-bold text-xl">Data Driver</div>
                        </div>
                        ...
                        <div class="p-0 " style="width:100%;overflow-x:auto;">
                            <div class="flex flex-col sm:flex-row justify-between items-center mb-4">
                                <div class="md:mt-0 sm:flex sm:space-x-4 w-full">
                                    <!-- Form untuk entries -->
                                    <x-show-entries :route="route('drivers.index')" :search="request()->search">
                                    </x-show-entries>
                                </div>
                                    ...
                                    <table class="table-auto w-full text-sm text-left text-gray-500 border">
                                        <thead class="text-md font-bold text-gray-700 uppercase">
                                            <tr>
                                                ...
                                                <th scope="col" class="px-3 py-2 text-center bg-gray-100">NIK</th>
                                                <th scope="col" class="px-3 py-2 text-center w-80">NAMA</th>
                                                <th scope="col" class="px-3 py-2 text-center bg-gray-100">NO
                                                    HANDPHONE</th>
                                                ...
                                                <th scope="col" class="px-3 py-2 text-center bg-gray-100">KTP</th>
                                                <th scope="col" class="px-3 py-2 text-center ">AKSI</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @php
                                                $no = $drivers->firstItem();
                                            @endphp
                                            @forelse($drivers as $i)
                                                <tr class="bg-white border dark:bg-gray-800 dark:border-gray-700">
                                                    <td class="px-3 py-2 text-center">{{ $no++ }}</td>
                                                    <td class="px-3 py-2 bg-gray-100">
                                                        {{ $i->nik }}
                                                    </td>
                                                    <td class="px-3 py-2 text-wrap">{{ $i->name }}</td>
                                                    <td class="px-3 py-2 text-center bg-gray-100">{{ $i->phoneNumber }}
                                                    </td>
                                                    <td class="px-3 py-2">{{ $i->email }}
                                                    </td>
                                                    ...
                                                    <td class="px-3 py-2 text-center flex items-center justify-center">
                                                        <img src="{{ url('/driver/img/', $i->photo) }}"
                                                            alt="{{ $i->name }}" class="w-20 rounded-full">
                                                    </td>
                                                    <td class="px-3 py-2 text-center bg-gray-100">
                                                        <a href="{{ url('/driver/ktp/', $i->photoKtp) }}"
                                                            target="_blank"
                                                            class="border border-sky-500 text-sky-500 hover:bg-sky-100 flex items-center justify-center rounded-xl h-10 w-10 text-md">
                                                            <i class="fi fi-rr-vision-target mt-1"></i>
                                                        </a>
                                                    </td>
                                                    <td class="px-3 py-2 text-center">
                                                        <div class="flex justify-center items-center gap-2">
                                                            <a href="{{ route('drivers.edit', $i->id) }}"
                                                            ...
                                                            <a onclick="return dataDelete('{{ $i->id }}','{{ $i->route }}')"
                                                                class="border border-red-500 text-red-500 hover:bg-red-100 flex items-center justify-center rounded-xl h-10 w-10 text-md">
                            ...
                            <div class="mt-4">
                                @if ($drivers->hasPages())
                                    {{ $drivers->appends(request()->query())->links('vendor.pagination.custom') }}
                                @else
                                    <div class="flex items-center justify-between">
                                        <nav class="flex justify-start">
                                            <div class="text-sm flex gap-1">
                                                ...
                                                <div>to</div>
                                                <div class="font-bold">{{ count($drivers) }}</div>
                                                <div>of</div>
                                                <div class="font-bold">{{ count($drivers) }}</div>
                                                <div>entries</div>
                                            </div>

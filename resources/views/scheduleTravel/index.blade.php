                ...
                <div class="bg-white px-12 pt-5 pb-6 rounded-3xl border shadow-xl w-full">
                    <div class="flex items-center gap-3 justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="font-bold text-xl">Schedule Travel</div>
                        </div>
                        <div class="flex items-center">
                            <a href="{{ route('schedule.create') }}"
                                class="border border-sky-500 text-sky-500 px-6 py-2 font-bold rounded-xl hover:bg-sky-100"><i
                                    class="fi fi-rr-add mr-1"></i> <span>Tambah Schedule</span></a>
                        </div>
                    </div>
                        ...
                        <div class="p-0 " style="width:100%;overflow-x:auto;">
                            <div class="flex flex-col sm:flex-row justify-between items-center mb-4">
                                <div class="md:mt-0 sm:flex sm:space-x-4 w-full">
                                    <!-- Form untuk entries -->
                                    <x-show-entries :route="route('schedule.index')" :search="request()->search">
                                    </x-show-entries>
                                </div>
                                    ...
                                    <table class="table-auto w-full text-sm text-left text-gray-500 border">
                                        <thead class="text-md font-bold text-gray-700 uppercase">
                                            <tr>
                                                ...
                                                <th scope="col" class="px-3 py-2 text-center bg-gray-100">Hari
                                                </th>
                                                <th scope="col" class="px-3 py-2 text-center">Rute</th>
                                                <th scope="col" class="px-3 py-2 text-center bg-gray-100">
                                                    Transportasi</th>
                                                    <th scope="col" class="px-3 py-2 text-center bg-gray-100">Driver</th>
                                                <th scope="col" class="px-3 py-2 text-center">Harga</th>
                                                <th scope="col" class="px-3 py-2 text-center bg-gray-100">Seat</th>
                                                <th scope="col" class="px-3 py-2 text-center">AKSI</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @php
                                                $no = $schedule->firstItem();
                                            @endphp
                                            @forelse($schedule as $i)
                                                <tr class="bg-white border dark:bg-gray-800 dark:border-gray-700">
                                                    ...
                                                    <td class="px-3 py-2 text-center bg-gray-100">{{ $i->name }}
                                                    </td>
                                                    <td class="px-3 py-2 text-center">{{ $i->route_price }}</td>
                                                    <td class="px-3 py-2 text-center bg-gray-100">{{ $i->seats }}
                                                    <td class="px-3 py-2 text-center flex gap-2 justify-center">
                                                        <a href="{{ route('schedule.edit', $i->id) }}"
                                                        ...
                                                        <button
                                                            onclick="return dataDelete('{{ $i->id }}','{{ $i->model }}')"
                            ...
                            <div class="mt-4">
                                @if ($schedule->hasPages())
                                    {{ $schedule->appends(request()->query())->links('vendor.pagination.custom') }}
                                @else
                                    <div class="flex items-center justify-between">
                                        <nav class="flex justify-start">
                                            <div class="text-sm flex gap-1">
                                                ...
                                                <div>to</div>
                                                <div class="font-bold">{{ count($schedule) }}</div>
                                                <div>of</div>
                                                <div class="font-bold">{{ count($schedule) }}</div>
                                                <div>entries</div>
                                            </div>

                ...
                <div class="bg-white px-12 pt-5 pb-6 rounded-3xl border shadow-xl w-full">
                    <div class="flex items-center gap-3 justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="font-bold text-xl">Transportasi Travel</div>
                        </div>
                        <div class="flex items-center">
                            <a href="{{ route('transportationsTravel.create') }}"
                        ...
                        <div class="p-0 " style="width:100%;overflow-x:auto;">
                            <div class="flex flex-col sm:flex-row justify-between items-center mb-4">
                                <div class="md:mt-0 sm:flex sm:space-x-4 w-full">
                                    <!-- Form untuk entries -->
                                    <x-show-entries :route="route('transportationsTravel.index')" :search="request()->search">
                                    </x-show-entries>
                                </div>

                                <div class="sm:ml-16 sm:mt-0 sm:flex sm:space-x-4 sm:flex-none">
                                    <form action="{{ route('transportationsTravel.index') }}" method="GET"
                                    ...
                                    <table class="table-auto w-full text-sm text-left text-gray-500 border">
                                        <thead class="text-md font-bold text-gray-700 uppercase">
                                            <tr>
                                                ...
                                                <th scope="col" class="px-3 py-2 text-center bg-gray-100">MODEL</th>
                                        ...
                                        <tbody>
                                            @php
                                                $no = $transportationDetailTravel->firstItem();
                                            @endphp
                                            @forelse($transportationDetailTravel as $i)
                                                <tr class="bg-white border dark:bg-gray-800 dark:border-gray-700">
                                                    ...
                                                    <td class="px-3 py-2 text-center flex gap-2 justify-center">
                                                        <a href="{{ route('transportationsTravel.edit', $i->codeDetailTransportation) }}"
                            ...
                            <div class="mt-4">
                                @if ($transportationDetailTravel->hasPages())
                                    {{ $transportationDetailTravel->appends(request()->query())->links('vendor.pagination.custom') }}
                                @else
                                    <div class="flex items-center justify-between">
                                        <nav class="flex justify-start">
                                            <div class="text-sm flex gap-1">
                                                ...
                                                <div>to</div>
                                                <div class="font-bold">{{ count($transportationDetailTravel) }}</div>
                                                <div>of</div>
                                                <div class="font-bold">{{ count($transportationDetailTravel) }}</div>
                                                <div>entries</div>
                                            </div>

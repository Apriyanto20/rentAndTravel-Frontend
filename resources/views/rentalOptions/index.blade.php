<x-app-layout>
            ...
            <div class="flex items-start justify-between gap-5">
                <div class="bg-white px-12 pt-5 pb-6 rounded-3xl border shadow-xl w-full">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="font-bold text-xl">Form Input Data Opsi Rental</div>
                    </div>
                    <hr class="mb-4 mt-3 border-2 rounded-xl">
                    <div>
                        <form action="{{ route('rentalOptions.store') }}" method="post" id="rentalOptionForm">
                            @csrf
                            <div class="mb-5">
                                <div
                                    class="mb-2 text-md font-medium text-gray-900 dark:text-white flex items-center gap-3">
                                    <i class="fi fi-rr-display-code flex"></i> <span>Kode Opsi Rental <span
                                            class="text-red-500">*</span></span>
                                </div>
                                <input type="text" name="codeRentalOption" id="codeRentalOption"
                                    class="w-full border border-gray-300 rounded-3xl px-4"
                                    placeholder="Kode Rental Opsi ....." value="{{ $codeRentalOptions }}" readonly>
                                <p id="error-codeRentalOption" class="mt-2 text-sm text-red-500 hidden">Kode wajib
                                    diisi.</p>
                            </div>
                            <div class="mb-5">
                                <div
                                    class="mb-2 text-md font-medium text-gray-900 dark:text-white flex items-center gap-3">
                                    <i class="fi fi-rr-dot-circle flex"></i> <span>Opsi Rental<span
                                            class="text-red-500">*</span></span>
                                </div>
                                <input type="text" name="option" id="option"
                                    class="w-full border border-gray-300 rounded-3xl px-4"
                                    placeholder="Opsi Rental....." oninput="this.value = this.value.toUpperCase();">
                                <p id="error-option" class="mt-2 text-sm text-red-500 hidden">Opsi wajib
                                    diisi.</p>
                            </div>
                            <div class="flex">
                                <button
                ...
                <div class="bg-white px-12 pt-5 pb-6 rounded-3xl border shadow-xl w-full">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="font-bold text-xl">Data Opsi Rental</div>
                    </div>
                    <hr class="mb-4 mt-3 border-2 rounded-xl">
                        ...
                        <div class="p-0 " style="width:100%;overflow-x:auto;">
                            <div class="flex flex-col sm:flex-row justify-between items-center mb-4">
                                <div class="md:mt-0 sm:flex sm:space-x-4 w-full">
                                    <!-- Form untuk entries -->
                                    <x-show-entries :route="route('rentalOptions.index')" :search="request()->search">
                                    </x-show-entries>
                                </div>

                                <div class="sm:ml-16 sm:mt-0 sm:flex sm:space-x-4 sm:flex-none">
                                    <form action="{{ route('rentalOptions.index') }}" method="GET"
                                            ...
                                            @forelse($rentalOptions as $i)
                                                <tr class="bg-white border dark:bg-gray-800 dark:border-gray-700">
                                                    ...
                                                    <td class="px-3 py-2 text-center bg-gray-100">
                                                        ...
                                                        <button
                                                            onclick="return dataDelete('{{ $i->id }}','{{ $i->option }}')"
                            ...
                            <div class="mt-4">
                                @if ($rentalOptions->hasPages())
                                    {{ $rentalOptions->appends(request()->query())->links('vendor.pagination.custom') }}
                                @else
                                    <div class="flex items-center justify-between">
                                        <nav class="flex justify-start">
                                            <div class="text-sm flex gap-1">
                                                ...
                                                <div>to</div>
                                                <div class="font-bold">{{ count($rentalOptions) }}</div>
                                                <div>of</div>
                                                <div class="font-bold">{{ count($rentalOptions) }}</div>
                                                <div>entries</div>
                                            </div>
            ...
            <div class="w-full md:w-1/4 relative bg-white rounded-lg shadow mx-5">
                ...
                <form method="POST" id="formSourceModal">
                    ...
                    <div class="flex flex-col p-4 space-y-2"> <!-- Jarak antar input dikurangi -->
                        <div>
                            <label for="codeRentalOptions"
                                class="block mb-1 text-sm font-medium text-gray-900 dark:text-white">Kode
                                Opsi Rental</label>
                            <input id="codeRentalOptions" name="codeRentalOption"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                placeholder="Kode Opsi Rental ..." required readonly>
                        </div>
                        <div>
                            <label for="options"
                                class="block mb-1 text-sm font-medium text-gray-900 dark:text-white">Opsi Rental</label>
                            <input id="options" name="option"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                placeholder="Opsi Rental ..." oninput="this.value = this.value.toUpperCase();"
                                required>
                            </input>
    ...
    <script>
        const form = document.getElementById('rentalOptionForm');
        ...
        form.addEventListener('submit', function(e) {
            ...
            const codeRentalOption = document.getElementById('codeRentalOption');
            const errorCodeRentalOption = document.getElementById('error-codeRentalOption');
            if (codeRentalOption.value === '') {
                errorCodeRentalOption.classList.remove('hidden');
                isValid = false;
            } else {
                errorCodeRentalOption.classList.add('hidden');
            }

            const option = document.getElementById('option');
            const errorOption = document.getElementById('error-option');
            if (option.value === '') {
                errorOption.classList.remove('hidden');
                isValid = false;
            } else {
                errorOption.classList.add('hidden');
            }
    ...
    <script>
        const editSourceModal = (button) => {
            ...
            const codeRentalOption = button.dataset.code;
            const option = button.dataset.option;
            let url = "{{ route('rentalOptions.update', ':id') }}".replace(':id', id);
            console.log(codeRentalOption);


            document.getElementById('title_source').innerText = `${option}`;
            document.getElementById('codeRentalOptions').value = codeRentalOption;
        ...
        const dataDelete = async (id, option) => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    await axios.post(`/rentalOptions/${id}`, {
                            '_method': 'DELETE',

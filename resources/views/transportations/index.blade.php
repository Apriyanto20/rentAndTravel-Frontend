<x-app-layout>
            ...
            <div class="flex items-start justify-between gap-5">
                <div class="bg-white px-12 pt-5 pb-6 rounded-3xl border shadow-xl w-full">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="font-bold text-xl">Form Input Data Transportasi</div>
                    </div>
                    <hr class="mb-4 mt-3 border-2 rounded-xl">
                    <div>
                        <form action="{{ route('transportations.store') }}" method="post" id="transportationsForm">
                            @csrf
                            <div class="mb-5">
                                <div
                                    class="mb-2 text-md font-medium text-gray-900 dark:text-white flex items-center gap-3">
                                    <i class="fi fi-rr-display-code flex"></i> <span>Kode Transportasi <span
                                            class="text-red-500">*</span></span>
                                </div>
                                <input type="text" name="codeTransportation" id="codeTransportation"
                                    class="w-full border border-gray-300 rounded-3xl px-4"
                                    placeholder="Kode Transportasi ....." value="{{ $codeTransportation }}" readonly>
                                <p id="error-codeTransportation" class="mt-2 text-sm text-red-500 hidden">Kode wajib
                                    diisi.</p>
                            </div>
                            <div class="mb-5">
                                <div
                                    class="mb-2 text-md font-medium text-gray-900 dark:text-white flex items-center gap-3">
                                    <i class="fi fi-rr-shipping-fast flex"></i> <span>Transportasi <span
                                            class="text-red-500">*</span></span>
                                </div>
                                <input type="text" name="transportation" id="transportation"
                                    class="w-full border border-gray-300 rounded-3xl px-4"
                                    placeholder="Transportasi ....." oninput="this.value = this.value.toUpperCase();">
                                <p id="error-transportation" class="mt-2 text-sm text-red-500 hidden">Transportasi wajib
                                    diisi.</p>
                            </div>
                            <div class="flex">
                                <button
                ...
                <div class="bg-white px-12 pt-5 pb-6 rounded-3xl border shadow-xl w-full">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="font-bold text-xl">Data Transportasi</div>
                    </div>
                    <hr class="mb-4 mt-3 border-2 rounded-xl">
                        ...
                        <div class="p-0 " style="width:100%;overflow-x:auto;">
                            <div class="flex flex-col sm:flex-row justify-between items-center mb-4">
                                <div class="md:mt-0 sm:flex sm:space-x-4 w-full">
                                    <!-- Form untuk entries -->
                                    <x-show-entries :route="route('transportations.index')" :search="request()->search">
                                    </x-show-entries>
                                </div>

                                <div class="sm:ml-16 sm:mt-0 sm:flex sm:space-x-4 sm:flex-none">
                                    <form action="{{ route('transportations.index') }}" method="GET"
                            ...
                            <div class="relative overflow-x-auto rounded-lg shadow-lg">
                                <div class="overflow-x-auto">
                                    <table class="table-auto w-full text-sm text-left text-gray-500 border">
                                        ...
                                        <tbody>
                                            @php
                                                $no = $transportations->firstItem();
                                            @endphp
                                            @forelse($transportations as $i)
                                                <tr class="bg-white border dark:bg-gray-800 dark:border-gray-700">
                                                    <td class="px-3 py-2 text-center">{{ $no++ }}</td>
                                                    <td class="px-3 py-2 text-center bg-gray-100">
                                                        {{ $i->codeTransportation }}
                                                    </td>
                                                    <td class="px-3 py-2 text-center">{{ $i->transportation }}</td>
                                                    <td class="px-3 py-2 text-center bg-gray-100">
                                                        <button type="button" data-id="{{ $i->id }}"
                                                            data-modal-target="sourceModal"
                                                            data-code="{{ $i->codeTransportation }}"
                                                        ...
                                                        <button
                                                            onclick="return dataDelete('{{ $i->id }}','{{ $i->transportation }}')"
                            ...
                            <div class="mt-4">
                                @if ($transportations->hasPages())
                                    {{ $transportations->appends(request()->query())->links('vendor.pagination.custom') }}
                                @else
                                    <div class="flex items-center justify-between">
                                        <nav class="flex justify-start">
                                            <div class="text-sm flex gap-1">
                                                ...
                                                <div>to</div>
                                                <div class="font-bold">{{ count($transportations) }}</div>
                                                <div>of</div>
                                                <div class="font-bold">{{ count($transportations) }}</div>
                                                <div>entries</div>
                                            </div>
            ...
            <div class="w-full md:w-1/4 relative bg-white rounded-lg shadow mx-5">
                ...
                <form method="POST" id="formSourceModal">
                    ...
                    <div class="flex flex-col p-4 space-y-2"> <!-- Jarak antar input dikurangi -->
                        <div>
                            <label for="codeTransportations"
                                class="block mb-1 text-sm font-medium text-gray-900 dark:text-white">Kode
                                Transportasi</label>
                            <input id="codeTransportations" name="codeTransportation"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                placeholder="Kode Transportasi ..." required readonly>
                        </div>
                        <div>
                            <label for="transportations"
                                class="block mb-1 text-sm font-medium text-gray-900 dark:text-white">Transportasi</label>
                            <input id="transportations" name="transportation"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                placeholder="Transportasi ..." oninput="this.value = this.value.toUpperCase();"
                                required>
                            </input>
    ...
    <script>
        const form = document.getElementById('transportationsForm');
        ...
        form.addEventListener('submit', function(e) {
            ...
            const codeTransportation = document.getElementById('codeTransportation');
            const errorCodeTransportation = document.getElementById('error-codeTransportation');
            if (codeTransportation.value === '') {
                errorCodeTransportation.classList.remove('hidden');
                isValid = false;
            } else {
                errorCodeTransportation.classList.add('hidden');
            }

            const transportation = document.getElementById('transportation');
            const errorTransportation = document.getElementById('error-transportation');
            if (transportation.value === '') {
                errorTransportation.classList.remove('hidden');
                isValid = false;
            } else {
                errorTransportation.classList.add('hidden');
            }
    ...
    <script>
        const editSourceModal = (button) => {
            ...
            const codeTransportation = button.dataset.code;
            const transportation = button.dataset.transportation;
            let url = "{{ route('transportations.update', ':id') }}".replace(':id', id);
            console.log(codeTransportation);


            document.getElementById('title_source').innerText = `${transportation}`;
            document.getElementById('codeTransportations').value = codeTransportation;
            document.getElementById('transportations').value = transportation;
        ...
        const dataDelete = async (id, pukul) => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    await axios.post(`/transportations/${id}`, {
                            '_method': 'DELETE',

...
class TransportationsRentalMotorcyleController extends Controller
{
    ...
    public function show(string $id)
    {
        try {
            $auth = Auth::user()->role;
            $member = Members::where('email', Auth::user()->email)->first();
            $nik = $member->nik;
            if ($auth !== 'A') {
                $transaksi = TransactionsRental::where('memberCode', $nik)->get();
                $hasil = $transaksi->contains(function ($item) {
                    return $item->status === 'SELESAI';
                }) ? 'SELESAI' : 'BELUM SELESAI';
            }
            $transportationsRental = TransportationsRentalDetail::where('codeDetailTransportation', $id)->first();
            return view('transportationsRental.showMotorcycle')->with([
